...
#define CHECK_ARG(x, y) \
			...
			CHECK_ARG_FLOAT("globals.hitscan_velocity = ", settings->hitscan_projectile_velocity);
			CHECK_ARG_FLOAT("globals.magnetism_friction = ", settings->magnetism_friction);
			CHECK_ARG_FLOAT("globals.magnetism_adhesion = ", settings->magnetism_adhesion);
			CHECK_ARG_FLOAT("globals.melee_angles_y = ", settings->melee_angles_y);
			CHECK_ARG_FLOAT("globals.melee_angles_p = ", settings->melee_angles_p);
			CHECK_ARG_FLOAT("globals.melee_depth = ", settings->melee_depth);
			CHECK_ARG_FLOAT("spawns.max_random_spawn_bias = ", settings->max_random_spawn_bias);
			CHECK_ARG_FLOAT("player.run_acceleration = ", settings->player_run_acceleration);
			CHECK_ARG_FLOAT("player.airborn_acceleration = ", settings->player_airborn_acceleration);
			CHECK_ARG("player.turn_off_melee_lunge = ", settings->player_turn_off_melee_lunge);
			CHECK_ARG("player.turn_off_spawn_protection = ", settings->player_turn_off_spawn_protection);
			CHECK_ARG("player.turn_off_overshield_shader = ", settings->player_turn_off_overshield_shader);
			CHECK_ARG_FLOAT("battle_rifle.autoaim_angle = ", settings->br_autoaim_angle);
			CHECK_ARG_FLOAT("battle_rifle.autoaim_range = ", settings->br_autoaim_range);
			CHECK_ARG_FLOAT("battle_rifle.fire_recovery_time = ", settings->br_fire_recovery_time);
			CHECK_ARG_FLOAT("battle_rifle.camo_ding = ", settings->br_camo_ding);
			CHECK_ARG_FLOAT("battle_rifle.camo_regrowth_rate = ", settings->br_camo_regrowth_rate);
			CHECK_ARG("battle_rifle.turn_off_crosshair_dot = ", settings->br_turn_off_crosshair_dot);
			CHECK_ARG("battle_rifle.turn_off_contrail = ", settings->br_turn_off_contrail);
			CHECK_ARG_FLOAT("sniper.autoaim_angle = ", settings->snipe_autoaim_angle);
			CHECK_ARG_FLOAT("sniper.camo_ding =", settings->snipe_camo_ding);
			CHECK_ARG_FLOAT("sniper.camo_regrowth_rate =", settings->snipe_camo_regrowth_rate);
			CHECK_ARG_FLOAT("needler.rounds_per_second_min = ", settings->n_rounds_per_second_min);
			CHECK_ARG_FLOAT("needler.rounds_per_second_max = ", settings->n_rounds_per_second_max);
			CHECK_ARG_FLOAT("needler.initial_velocity = ", settings->n_initial_velocity);
			CHECK_ARG_FLOAT("needler.final_velocity = ", settings->n_final_velocity);
			CHECK_ARG_FLOAT("needler.camo_ding = ", settings->n_camo_ding);
			CHECK_ARG_FLOAT("needler.camo_regrowth_rate = ", settings->n_camo_regrowth_rate);
			CHECK_ARG_FLOAT("plasma_rifle.stun = ", settings->pr_stun);
			CHECK_ARG_FLOAT("plasma_rifle.max_stun = ", settings->pr_max_stun);
			CHECK_ARG_FLOAT("plasma_rifle.stun_time = ", settings->pr_stun_time);
			CHECK_ARG_FLOAT("plasma_rifle.initial_velocity = ", settings->pr_initial_velocity);
			CHECK_ARG_FLOAT("plasma_rifle.final_velocity = ", settings->pr_final_velocity);
			CHECK_ARG_FLOAT("plasma_rifle.max_error_angle = ", settings->pr_max_error_angle);
			CHECK_ARG_FLOAT("plasma_rifle.camo_ding = ", settings->pr_camo_ding);
			CHECK_ARG_FLOAT("plasma_rifle.camo_regrowth_rate = ", settings->pr_camo_regrowth_rate);
			CHECK_ARG_FLOAT("rockets.initial_velocity = ", settings->r_initial_velocity);
			CHECK_ARG_FLOAT("rockets.damage_upper_bound_min = ", settings->r_damage_upper_bound_min);
			CHECK_ARG_FLOAT("rockets.damage_upper_bound_max = ", settings->r_damage_upper_bound_max);
			CHECK_ARG_FLOAT("rockets.camo_ding = ", settings->r_camo_ding);
			CHECK_ARG_FLOAT("rockets.camo_regrowth_rate = ", settings->r_camo_regrowth_rate);
			CHECK_ARG_FLOAT("rockets.radius_min = ", settings->r_radius_min);
			CHECK_ARG_FLOAT("rockets.radius_max = ", settings->r_radius_max);
			CHECK_ARG_FLOAT("smg.rounds_per_second_min = ", settings->smg_rounds_per_second_min);
			CHECK_ARG_FLOAT("smg.rounds_per_second_max = ", settings->smg_rounds_per_second_max);
			CHECK_ARG_FLOAT("smg.minimum_error = ", settings->smg_minimum_error);
			CHECK_ARG_FLOAT("smg.damage_min = ", settings->smg_damage_min);
			CHECK_ARG_FLOAT("smg.damage_max = ", settings->smg_damage_max);
			CHECK_ARG_FLOAT("smg.camo_ding = ", settings->smg_camo_ding);
			CHECK_ARG_FLOAT("smg.camo_regrowth_rate = ", settings->smg_camo_regrowth_rate);
			CHECK_ARG_FLOAT("magnum.fire_recovery_time = ", settings->mag_fire_recovery_time);
			CHECK_ARG_FLOAT("magnum.error_angle_min = ", settings->mag_error_angle_min);
			CHECK_ARG_FLOAT("magnum.error_angle_max = ", settings->mag_error_angle_max);
			CHECK_ARG_FLOAT("magnum.damage_lower_bound = ", settings->mag_damage_lower_bound);
			CHECK_ARG_FLOAT("magnum.damage_upper_bound_min = ", settings->mag_damage_upper_bound_min);
			CHECK_ARG_FLOAT("magnum.damage_upper_bound_max = ", settings->mag_damage_upper_bound_max);
			CHECK_ARG_FLOAT("magnum.camo_ding = ", settings->mag_camo_ding);
			CHECK_ARG_FLOAT("magnum.camo_regrowth_rate = ", settings->mag_camo_regrowth_rate);
			CHECK_ARG_FLOAT("plasma_pistol.initial_velocity = ", settings->pp_initial_velocity);
			CHECK_ARG_FLOAT("plasma_pistol.final_velocity = ", settings->pp_final_velocity);
			CHECK_ARG_FLOAT("plasma_pistol.guided_angular_velocity_lower = ", settings->pp_guided_angular_velocity_lower);
			CHECK_ARG_FLOAT("plasma_pistol.guided_angular_velocity_upper = ", settings->pp_guided_angular_velocity_upper);
			CHECK_ARG_FLOAT("plasma_pistol.heat_per_charged_shot = ", settings->pp_heat_per_charged_shot);
			CHECK_ARG_FLOAT("plasma_pistol.camo_ding = ", settings->pp_camo_ding);
			CHECK_ARG_FLOAT("plasma_pistol.camo_regrowth_rate = ", settings->pp_camo_regrowth_rate);
			CHECK_ARG_FLOAT("carbine.autoaim_angle = ", settings->car_autoaim_angle);
			CHECK_ARG_FLOAT("carbine.autoaim_range = ", settings->car_autoaim_range);
			CHECK_ARG_FLOAT("carbine.fire_recovery_time = ", settings->car_fire_recovery_time);
			CHECK_ARG_FLOAT("carbine.error_angle = ", settings->car_error_angle);
			CHECK_ARG_FLOAT("carbine.damage = ", settings->car_damage);
			CHECK_ARG_FLOAT("carbine.camo_ding = ", settings->car_camo_ding);
			CHECK_ARG_FLOAT("carbine.camo_regrowth_rate = ", settings->car_camo_regrowth_rate);
			CHECK_ARG_FLOAT("shotgun.projectiles_per_shot = ", settings->sg_projectiles_per_shot);
			CHECK_ARG_FLOAT("shotgun.error_angle_min = ", settings->sg_error_angle_min);
			CHECK_ARG_FLOAT("shotgun.error_angle_max = ", settings->sg_error_angle_max);
			CHECK_ARG_FLOAT("shotgun.damage_lower_bound = ", settings->sg_damage_lower_bound);
			CHECK_ARG_FLOAT("shotgun.damage_upper_bound_min = ", settings->sg_damage_upper_bound_min);
			CHECK_ARG_FLOAT("shotgun.damage_upper_bound_max = ", settings->sg_damage_upper_bound_max);
			CHECK_ARG_FLOAT("shotgun.camo_ding = ", settings->sg_camo_ding);
			CHECK_ARG_FLOAT("shotgun.camo_regrowth_rate = ", settings->sg_camo_regrowth_rate);
			CHECK_ARG_FLOAT("frag.arming_time = ", settings->f_arming_time);
			CHECK_ARG_FLOAT("frag.radius_to = ", settings->f_radius_to);
			CHECK_ARG_FLOAT("frag.arming_time = ", settings->pg_arming_time);
		}
	}
	else
	{
...
#pragma region Apply Weapon Balances
	...
	*(float*)(address_offset + 0xD80FFC) = settings->max_random_spawn_bias;
